...
// N  = Normal (normal)
// Rm = which is the direction that a perfectly reflected ray of light would take from this point on the surface
//  - Rm = 2(Lm * N)N-Lm
...
vec4 calcLightColour(vec3 light_colour, float light_intensity, vec3 position, vec3 to_light_dir, vec3 normal) {
    ...
    /*vec3 Rm = 2 * dot(position, normal) * normalize(normal) - normalize(position);

    vec3 diffuse = Kd * dot(position, normal) * light_intensity;
    vec3 specular = Ks * pow(dot(Rm, to_light_dir), material.reflectance) * light_intensity;
    */

    // Diffuse Light
    float diffuseFactor = max(dot(normal, to_light_dir), 0.0);
    vec4 diffuseColour = vec4(light_colour, 1.0) * light_intensity * vec4(Kd, 1.0) * diffuseFactor;
    ...
    vec4 specColour = light_intensity  * ( vec4(Ks, 1.0) * specularFactor ) * vec4(light_colour, 1.0);
