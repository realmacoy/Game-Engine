package com.github.theobjop.engine.loader.md5;

import com.github.theobjop.engine.Util;
import com.github.theobjop.engine.game.anim.AnimGameObject;
import com.github.theobjop.engine.game.anim.AnimVertex;
...
public class MD5Loader {
     ...
     * @throws Exception
     */
    public static AnimGameObject process(MD5Model md5Model, MD5AnimModel animModel, Vector3f defaultColour) {
        ...
        AnimGameObject result = new AnimGameObject(meshes, animatedFrames, invJointMatrices);
        return result;
    }
    ...
    private static Mesh createMesh(List<AnimVertex> vertices, List<Integer> indices) {
        ...
        float[] positionsArr = Util.listToArray(positions);
        float[] textCoordsArr = Util.listToArray(textCoords);
        float[] normalsArr = Util.listToArray(normals);
        int[] indicesArr = Util.listIntToArray(indices);
        int[] jointIndicesArr = Util.listIntToArray(jointIndices);
        float[] weightsArr = Util.listToArray(weights);
    ...
    private static void handleTexture(Mesh mesh, MD5Mesh md5Mesh, Vector3f defaultColour) {
        ...
        if (texturePath != null && texturePath.length() > 0) {
            ...
            if (pos > 0) {
                ...
                if (Util.existsResourceFile(normalMapFileName)) {
