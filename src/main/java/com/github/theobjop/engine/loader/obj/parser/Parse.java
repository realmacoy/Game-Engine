package com.github.theobjop.engine.loader.obj.parser;
...
public class Parse {
    ...
    public Parse(BuilderInterface builder, String filename) throws Exception {
    ...
    /**
     * Builder creation. If any error occurs, the program throws a runtime exception.
     * @param builder Class that extends the BuilderInterface
     * @param filename Filename of the model to be parsed
     * @return BuilderInterface that was created. Typecast it.
     */
    public static BuilderInterface SafeParse(Class<? extends BuilderInterface> builder, String filename) {
        BuilderInterface returnBuilder = null;
        try {
            returnBuilder = builder.newInstance();
            Parse parse = new Parse(returnBuilder, filename);
        } catch (Exception e) {
    ...
    private void parseObjFile(String objFilename) throws Exception {
        int lineCount = 0;
        FileReader fileReader = null;
        BufferedReader bufferedReader = null;

        if (!Util.existsResourceFile(objFilename))
            throw new Exception("Resource does not exist: " + objFilename);

        objFile = new File(Util.getResource(objFilename).toURI());
