...
import com.github.theobjop.engine.loader.obj.parser.BuilderInterface;
...
public class Build implements BuilderInterface {
    ...
    // triplets, each index referring to a vertex within ONE of the three array lists verticesG, verticesT
    // or verticesN, two faces might end up specifying the same combination.  Clearly this
...
//        log.log(INFO,"Parsed face=" + face);
    ...
    public void newMtl(String name) {
        currentMaterialBeingParsed = new Material();
        materialLib.put(name, currentMaterialBeingParsed);
    }

    public void setXYZ(int type, float x, float y, float z) {
        ReflectivityTransmiss rt = currentMaterialBeingParsed.ambient;
        if (type == MTL_KD) {
            rt = currentMaterialBeingParsed.diffuse;
        } else if (type == MTL_KS) {
            rt = currentMaterialBeingParsed.specular;
        } else if (type == MTL_TF) {
            rt = currentMaterialBeingParsed.transmissionFilter;
        }
    ...
    public void setNs(float exponent) {
        currentMaterialBeingParsed.setNs(exponent);
