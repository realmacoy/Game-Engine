...
/**
 * Created by 659bmw03 on 2/22/2017.
 */
public class Mesh {
    ...
    public Mesh(AIMesh mesh) {
        ...
        if (mesh.mTextureCoords(0) != null) {
            ...
            nglBufferData(GL_ARRAY_BUFFER, AIVector3D.SIZEOF * mesh.mVertices().remaining(),
                    mesh.mTextureCoords(0).address(), GL_STATIC_DRAW);
        }
        ...
        IntBuffer elementArrayBuffer  = BufferUtils.createIntBuffer(vertexCount);
        AIFace.Buffer facesBuffer = mesh.mFaces();
        for (int i = 0; i < faceCount; ++i) {
            AIFace face = facesBuffer.get(i);
            if (face.mNumIndices() != 3)
                throw new RuntimeException("Error trying to load mesh. Face does not have at least 3 indices.");
            elementArrayBuffer.put(face.mIndices());
        }

        vboId = glGenBuffers();
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vboId);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, elementArrayBuffer, GL_STATIC_DRAW);
        vboIdList.add(vboId);
    ...
    public Mesh(String objLocation, Material material) {
        this(OBJLoader.loadMesh(objLocation), material);
    }

    public Mesh(Mesh mesh, Material material) {
        this.vaoId = mesh.vaoId;
        this.vboIdList = mesh.vboIdList;
